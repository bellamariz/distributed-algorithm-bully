...
public class ElectionNodeStateNormal extends ElectionNodeState {
    ...
    public void handleAYUp(BullyMessage msg) {
        ...
        ctx.reliability++;
         ...
         if(ctx.reliability > ctx.RELIABILITY_THRESHOLD_MAX){
            ctx.inHysteresis = false;
        }

        if(ctx.reliability > ctx.MAX_RELIABILITY){
            ctx.reliability = ctx.MAX_RELIABILITY;
        }
    ...
    public void handleUpdate() {
        ...
        if (ctx.ID > ctx.coordinatorId && ctx.reliability > ctx.RELIABILITY_THRESHOLD_MIN){
            Tools.appendToOutput("Node " + ctx.ID + " minimum reliability achieved - calling elections\n");
            ctx.setState(States.ElectionCandidate);
            //ctx.coordinatorId = -1;
            ctx.c = -1;
        }
